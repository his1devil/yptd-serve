...
`:avatar 图片路径` 设置自己的头像：上传到对象存储，再写回账号。

`:update` 更新到最新发布版；命令行里是 `yptd update`。

## 版本与更新

版本号在状态栏左上角（`yptd v0.1.0`），`yptd --version` 也能看。发布版的版本
是 git tag，开发树用的是 Cargo.toml 里的版本。

启动时后台向 `<服务器同源>/dl/VERSION` 问一次最新版，几百字节，不挡界面，
失败就当没有新消息。有新版就在状态栏左上角挂一句「有新版 vX，:update 更新」，
不弹窗也不打断。

**不做静默自动更新**：客户端在前台跑着，二进制被悄悄换掉、人却还在用旧进程，
行为和预期对不上；更新一旦出错，用户手上会连个能用的客户端都没有。
提示 + 一条命令更稳，rustup、gh、bun 都是这个路子。

`yptd update` 下载对应架构的包、比对 `SHA256SUMS`、把二进制换到位。换的是
目录项（rename），正在跑的进程抓着旧 inode 不受影响，重开生效。
...
- **头像先看成员表，再看消息自带的那份。** OpenIM 在每条消息上盖一份发送者的
  `faceURL`，只认它的话，一个人换了头像，他以前说过的话还挂着旧头像。成员表是当前的，
  所以优先用它；私聊没有成员表，那时才退回消息自带的。
- **成员栏的头像顶掉了在线圆点。** 栏只有十几列宽，一人一行比图大更重要——花名册
  的价值就在于一眼扫完。变暗的色块和空心圆一样说明"人不在"，还顺带说明了是谁。
- **没设过头像的人也有头像。** 用已经按 id 哈希出来的作者色填一块 4×2，压上名字首字符。
  不需要网络也不需要图形协议——四列宽的照片用半块字符画出来本来也认不出是谁。
