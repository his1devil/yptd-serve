...
{"v": 1, "conversation_id": "sg_<groupID> 或 si_<a>_<b>", "kind": "message",
 "msg_id": "<clientMsgID>", "at": ["<userID>", "…"], "run_id": "<run_xxx>"}
```

- `conversation_id` 必填，是手机跳转的唯一依据；`si_` 的两个 id 按字典序。
- `kind`：`message`（默认）或 `run_result`（agent 对某人提问的回答，另带 `run_id`）。
- `msg_id` 是这条消息的 clientMsgID。**从通知进会话时本地往往还没同步到它**，客户端
  应该等这一条真的到达再跳（或者到达后高亮），否则用户先看到旧消息、新的那条几百毫秒
  后才追加进来。同一条消息重复到达时也靠它去重。
- `at` 是这条消息 @ 到的人（只含会收到这条通知的人），`["*"]` 表示 @所有人。**一条群
  消息只有一份通知文案**，服务端没法对被 @ 的人说「提到了你」、对别人说别的；客户端拿
  这个名单和自己的 userID 比，自己决定要不要加重。iOS 在 Notification Service
  Extension 里改写正文。
- 不放消息全文之外的敏感内容。
...
| --- | --- | --- |
| 私聊 | 推 | 标题 = 发送者昵称，正文 = 预览 |
| 群里的消息（不管有没有 @） | 推给全群 | 标题 = `#群名`，正文 = `发送者：预览` |
| **agent 在群里的主动播报**（行情、新闻、配置变更） | **不推** | — |
| agent 在群里回答某人的提问 | 推 | 同群消息，`ex.kind = run_result` |
| reaction / 自定义消息、输入状态、系统通知、agent 的「⏳ 正在处理…」占位 | 不推 | — |
| 发送者本人、agent 账号作为接收方 | 从名单里去掉 | — |

「回答」和「主动播报」靠 `offlinePushInfo.ex` 里的 `kind` 区分：agent 回答带
`run_result`（由 `SendText` 从消息 `ex` 里的 `{"yptd":"run"}` 推出来），行情、新闻这类
定时发的没有。分开是因为每天几十条播报会让人把整个 app 静音，而回答恰恰是最该响的。

预览按类型：文字原文、`[图片]`、`[语音]`、`[视频]`、`[文件] 名字`。文字会先剥一层
markdown（标题、强调、表格分隔行、围栏代码块 → `[代码]`、链接只留文字），再折叠空白、
截到 80 个字符——agent 的回答几乎都是 markdown，不剥的话锁屏上全是 `|---|---|`。
截断不会切在 emoji 组合序列中间。发送者昵称和群名也会折叠空白并截到 16 个字符：昵称
中间的换行躲得过注册时的校验，原样拼进标题会把通知撑成两行。

> **注意**：附件消息（只发图片不打字）两端客户端是以 101/106 + `ex` 里的 rich 结构发的，
> 不是 OpenIM 的 102–105，所以预览用的是客户端自己填的占位文字。iOS 发两张图是
> `[图片] [图片]`，桌面是 `[图片]×2`；桌面还没有视频种类，视频会显示成 `[文件] x.mp4`。
> 回调拿不到消息的 `ex`（OpenIM 的回调请求里没这个字段），服务端补不了这个差异——
> 要一致得两端客户端自己对齐占位文案。
